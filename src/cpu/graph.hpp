		...
		class graph
	{
		public:
			...
			graph(word_t maxedges, word_t maxnodes, u32 maxsols) : visited(2*maxnodes)
			{
				...
				adjlist = new word_t[2*MAXNODES];
				links   = new link[2*MAXEDGES];
				compressu = compressv = 0;
				sharedmem = false;
								 // extra one for current path
			...
			graph(word_t maxedges, word_t maxnodes, u32 maxsols, char *bytes) : visited(2*maxnodes)
			{
				...
				MAXSOLS = maxsols;
								 // index into links array
				adjlist = new (bytes) word_t[2*MAXNODES];
			...
			void add_edge(word_t u, word_t v)
			{
				...
				word_t vlink = nlinks++;
								 // avoid confusing links with NIL; guaranteed if bits in word_t > EDGEBITS + 1
				assert(vlink != NIL);
