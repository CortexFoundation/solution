...
namespace cuckoo_cpu
{
	...
	#ifdef SAVEEDGES
								 // assumes EDGEBITS <= 32
	const u32 ZBUCKETSIZE = NTRIMMEDZ * (CUCKOO_BIGSIZE + sizeof(u32));
	#else
	...
	class edgetrimmer
	{
		...
		void genVnodes(const u32 id, const u32 uorv)
		{
			...
			const u32   endux = NX * (id+1) / nthreads;
								 // matrix x == ux
			for (u32 ux = startux; ux < endux; ux++)
			{
				small.matrixu(0);
			...
			void trimrename1(const u32 id, const u32 round)
		{
			...
			{
				TRIMONV ? dst.matrixv(vx) : dst.matrixu(vx);
								 // sets each u16 entry to 0xffff
				memset(degs, 0xff, 2 * NYZ1);
				for (u32 ux = 0 ; ux < NX; ux++)
				{
					zbucket<ZBUCKETSIZE> &zb = TRIMONV ? buckets[ux][vx] : buckets[vx][ux];
					u32 *readbig = zb.words, *endreadbig = readbig + zb.size/sizeof(u32);
	...
	class solver_ctx
	{
		public:
			...
			std::bitset<NXY> uxymap;
								 // concatanation of all proof's indices
			std::vector<word_t> sols;
			...
			void solution(const u32 *us, u32 nu, const u32 *vs, u32 nv)
			{
				...
				while (nu--)
					...
					recordedge(ni++, us[(nu+1)&~1], us[nu|1]);
				while (nv--)
								 // u's in odd position; v's in even
					recordedge(ni++, vs[nv|1], vs[(nv+1)&~1]);
				print_log("\n");
				if (showcycle)
				{
				#ifndef SAVEEDGES
