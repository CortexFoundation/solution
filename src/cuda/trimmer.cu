...
namespace cuckoogpu
{
	...
	template < int maxOut > __global__ void Cuckaroo_SeedA (const siphash_keys & sipkeys, ulonglong4* __restrict__ buffer, int *__restrict__ indexes)
	{
		...
		const int FLUSHA2 = 2 * FLUSHA;
								 ...
								 // needs to be ulonglong4 aligned
		__shared__ uint2 tmp[NX][FLUSHA2];
		...
		const int col = group & YMASK;
								 // assuming THREADS_HAVE_EDGES checked
		const int loops = NEDGES / nthreads;
		for (int blk = 0; blk < loops; blk += EDGE_BLOCK_SIZE)
		{
			u32 nonce0 = gid * loops + blk;
			...
			for (e = 0; e < EDGE_BLOCK_SIZE; e++)
			{
				...
				int row = node0 >> YZBITS;
								 // assuming ROWS_LIMIT_LOSSES checked
	...
	template < int maxOut, typename EdgeOut > __global__ void Cuckoo_SeedA (const siphash_keys & sipkeys, ulonglong4 * __restrict__ buffer, int *__restrict__ indexes)
	{
		...
		const int FLUSHA2 = 2 * FLUSHA;
								 ...
								 // needs to be ulonglong4 aligned
		__shared__ EdgeOut tmp[NX][FLUSHA2];
	...
	const u32 NONPART_BITS = ZBITS - PART_BITS;
								 // 1 << ZBITS
	const u32 NONPART_MASK = (1 << NONPART_BITS) - 1;
								 // NZ / 8
	const int BITMAPBYTES = (NZ >> PART_BITS) / 4;
	...
	edgetrimmer::edgetrimmer (const trimparams _tp, u32 _deviceId, int _selected)
	{
		...
		const size_t bufferSize = sizeA + nonoverlap;
								 // ensure enough space for Round 1, / 2 is for 0.296 / 0.632 without expansion
		assert(bufferSize - sizeB >= sizeB / (tp.expand==2 ? 1 : 2));
		checkCudaErrors(cudaMalloc((void**)&bufferA, bufferSize));
		bufferAB = bufferA + nonoverlap;
		bufferB  = bufferA + bufferSize - sizeB;
								 // ensure NA is a 2 power
		assert((NA & (NA-1)) == 0);
								 // ensure disjoint source dest in SeedB
		assert(NA * NEPS_B * NRB1 >= NEPS_A * NX);
								 // equivalent to above
		assert(sizeA / NA <= nonoverlap);
								 // ensure disjoint source dest in 2nd phase of round 0
		assert(bufferA + sizeA * NRB2 / NX <= bufferB);
								 // ensure alignment of overlap
		assert(bufferA + sizeA == bufferB + sizeB * NRB2 / NX);
	...
	u32 edgetrimmer::trim (uint32_t device)
	{
		...
		{
								 // to .632
			Round<EDGES_A, EDGES_B*NRB1/NX><<<tp.trim.blocks*NRB1/NX, tp.trim.tpb, BITMAPBYTES>>>(0, part, *dipkeys, (uint2*)(bufferA+qA), (uint2*)(bufferA+sizeA), indexesE[0]+qE, indexesE[2]);
		}
		...
		{
								 // to .632
			Round<EDGES_A, EDGES_B*NRB2/NX><<<tp.trim.blocks*NRB2/NX, tp.trim.tpb, BITMAPBYTES>>>(0, part, *dipkeys, (uint2*)bufferA, (uint2*)bufferB, indexesE[0], indexesE[1]);
		}
		...
		{
								 // to .296
			Round2<EDGES_B*NRB2/NX, EDGES_B*NRB1/NX, EDGES_B/2><<<tp.trim.blocks, tp.trim.tpb, BITMAPBYTES>>>(1, part, *dipkeys, (uint2*)bufferB, (uint2*)bufferA, indexesE[1], indexesE[0]);
		}
		...
		{
								 // to .176
			Round<EDGES_B/2, EDGES_A/4><<<tp.trim.blocks, tp.trim.tpb, BITMAPBYTES>>>(2, part, *dipkeys, (uint2 *)bufferA, (uint2 *)bufferB, indexesE[0], indexesE[1]);
		}
		...
		{
								 // to .117
			Round<EDGES_A/4, EDGES_B/4><<<tp.trim.blocks, tp.trim.tpb, BITMAPBYTES>>>(3, part, *dipkeys, (uint2 *)bufferB, (uint2 *)bufferA, indexesE[1], indexesE[0]);
		}
